...
def create_colormap():
    ...
    return LinearColormap(
        colors, vmin=min(index), vmax=max(index), caption="Positiveness"
    )
...
def response_page(input_mood):
    try:
        ...
        playlist = spotify_mod.spotify_main(
            valency, danceability, energy, genre, song_list
        )
        ...
        singer_of_day_top_song = spotify_mod.get_artist_top_song(
            singer_of_day, song_country
        )
        ...
        response.set_cookie(
            "playlist", playlist_json, max_age=60 * 60 * 24 * 30
        )  # Cookie expires in 30 days
