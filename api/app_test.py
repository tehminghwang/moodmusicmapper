...
def test_response_page_valid_input():
    ...
    with app.test_client() as client:
        with patch("app.send_request") as mock_send_request, patch(
            "app.extract_values"
        ) as mock_extract_values, patch(
            "spotify_mod.spotify_main"
        ) as mock_spotify_main, patch(
            "database.display_phrase"
        ) as mock_display_phrase, patch(
            "database.song_of_day"
        ) as mock_song, patch(
            "app.create_map"
        ) as mock_create_map, patch(
            "spotify_mod.get_artist_top_song"
        ) as mock_artist, patch(
            "app.render_template"
        ) as mock_render:
            # Mock responses
